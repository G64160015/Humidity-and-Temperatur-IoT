...
const char MQTT_SUB_TOPIC[] = "SensorSuhu3";//"$aws/things/" THINGNAME "/shadow/update";
const char MQTT_PUB_TOPIC[] = "SensorSuhu3";//"$aws/things/" THINGNAME "/shadow/update";
...
void sendDataSensor(void)
{
    ...
    float t = dht.readTemperature()/1.00;
...
void loop()
{
  ...
  {
    client.loop();
    if (millis() - lastMillis > 10000)//send data every 10 sec
    {
      lastMillis = millis();
      sendDataSensor();
    }
